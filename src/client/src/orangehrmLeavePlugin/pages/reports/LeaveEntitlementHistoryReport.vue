...
<template>
  <div class="orangehrm-background-container">
    <oxd-table-filter :filter-title="$t('leave.leave_entitlement_history')">
...
export default {
  ...
  setup() {
    ...
    const headers = [
      {name: 'employeeName', title: $t('general.employee'), style: {flex: 1}},
      {
        name: 'leaveType',
        title: $t('leave.leave_type'),
        style: {flex: 1},
        accessor: (item) => item.leaveType?.name,
      },
      {
    ...
    const buildParams = () => {
      ...
      if (filters.value.employee?.id) {
        params.empNumber = filters.value.employee.id;
      }
